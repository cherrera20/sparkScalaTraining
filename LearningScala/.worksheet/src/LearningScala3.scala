object LearningScala3 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(225); 
  ...
  def upperString(s: String) : String = {
   	s.toUpperCase()
	};System.out.println("""upperString: (s: String)String""");$skip(37); 
	
	println(upperString("holamundo"));$skip(78); 
	
	def transformString(s: String, f: String => String) : String = {
		f(s)
	};System.out.println("""transformString: (s: String, f: String => String)String""");$skip(58); 
	
	println(transformString("test", s => s.toUpperCase()))}
