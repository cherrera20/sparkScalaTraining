object LearningScala4 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(285); 
 ...
 println(picardsShip._2);$skip(62); 
 
 val picardsShipComp = "Picard" -> "Enterprise-D" -> "test";System.out.println("""picardsShipComp  : ((String, String), String) = """ + $show(picardsShipComp ));$skip(29); 
 println(picardsShipComp._1);$skip(87); 
...
val backwardShips = shipList.map( (ship: String) => {ship.reverse});System.out.println("""backwardShips  : List[String] = """ + $show(backwardShips ));$skip(146); 
...
// go into more depth - but you've got enough to get through this course for now.
 ...
 val lstnums = List.range(1, 11);System.out.println("""lstnums  : List[Int] = """ + $show(lstnums ));$skip(54); 
 val lstmod = lstnums.filter( (x: Int) => x % 3 == 0);System.out.println("""lstmod  : List[Int] = """ + $show(lstmod ))}
